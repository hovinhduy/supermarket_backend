...
public interface PromotionHeaderRepository extends JpaRepository<PromotionHeader, Long> {
     ...
     * @return Page chứa danh sách PromotionHeader
     */
    Page<PromotionHeader> findByStartDateBetween(LocalDate startDate, LocalDate endDate, Pageable pageable);

    /**
     * Tìm chương trình đang hoạt động tại ngày hiện tại
     *
    ...
    Page<PromotionHeader> findActivePromotions(@Param("currentDate") LocalDate currentDate, Pageable pageable);
    ...
    @Query("SELECT ph FROM PromotionHeader ph WHERE ph.status IN ('UPCOMING', 'ACTIVE') " +
           "AND ph.startDate > :currentDate")
    Page<PromotionHeader> findUpcomingPromotions(@Param("currentDate") LocalDate currentDate, Pageable pageable);
     ...
     */
    @Query("SELECT ph FROM PromotionHeader ph WHERE ph.endDate < :currentDate")
    Page<PromotionHeader> findExpiredPromotions(@Param("currentDate") LocalDate currentDate, Pageable pageable);
    ...
    @Query("UPDATE PromotionHeader ph SET ph.status = 'EXPIRED' " +
           "WHERE ph.endDate < :currentDate AND ph.status IN ('ACTIVE', 'UPCOMING')")
    int updateExpiredPromotions(@Param("currentDate") LocalDate currentDate);
}
