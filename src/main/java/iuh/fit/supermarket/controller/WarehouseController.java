...
public class WarehouseController {
    ...
    @Operation(summary = "Lấy thông tin tồn kho theo biến thể", description = "Lấy thông tin tồn kho hiện tại của một biến thể sản phẩm")
    public ResponseEntity<ApiResponse<WarehouseDto>> getWarehouseByVariantId(
            @Parameter(description = "ID của biến thể sản phẩm", required = true) @PathVariable Long variantId) {

        log.info("API lấy thông tin tồn kho cho biến thể ID: {}", variantId);

        try {
            WarehouseDto warehouse = warehouseService.getWarehouseByVariantId(variantId);
    ...
    @Operation(summary = "Lấy số lượng tồn kho hiện tại", description = "Lấy số lượng tồn kho hiện tại của một biến thể sản phẩm")
    public ResponseEntity<ApiResponse<Integer>> getCurrentStock(
            @Parameter(description = "ID của biến thể sản phẩm", required = true) @PathVariable Long variantId) {

        log.info("API lấy số lượng tồn kho cho biến thể ID: {}", variantId);

        try {
            Integer currentStock = warehouseService.getCurrentStock(variantId);
    ...
    public ResponseEntity<ApiResponse<Boolean>> isStockAvailable(
        ...
        log.info("API kiểm tra tồn kho cho biến thể ID: {}, số lượng: {}", variantId, requiredQuantity);

        try {
            Boolean isAvailable = warehouseService.isStockAvailable(variantId, requiredQuantity);
    ...
    @Operation(summary = "Lấy lịch sử giao dịch theo biến thể", description = "Lấy lịch sử giao dịch kho của một biến thể sản phẩm")
    public ResponseEntity<ApiResponse<Page<WarehouseTransactionDto>>> getTransactionsByVariantId(
            @Parameter(description = "ID của biến thể sản phẩm", required = true) @PathVariable Long variantId,
            @Parameter(description = "Số trang (bắt đầu từ 0)") @RequestParam(defaultValue = "0") int page,
            @Parameter(description = "Số lượng bản ghi mỗi trang") @RequestParam(defaultValue = "20") int size) {
        ...
        try {
            ...
            Page<WarehouseTransactionDto> transactions = warehouseService.getTransactionsByVariantId(variantId,
                    pageable);
